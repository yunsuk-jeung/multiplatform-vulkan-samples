...
# (Ubuntu/Linux).
# Usage: ./install_dependencies_ubuntu.sh [debug|release]
#
# Currently installs the minimal build toolchain via apt, then builds spdlog
# from source. Other dependencies are added here as samples require them.
#
# Env:
...
# Install system packages: minimal build toolchain.
